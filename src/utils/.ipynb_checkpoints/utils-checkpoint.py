...
def get_init_metric_dict(task_type='classification'):
    """Get initial metric dictionary based on task type"""
    if task_type == 'regression':
        # For regression, initialize with large values since smaller is better
        return {'metric/best_clf_epoch': 0, 'metric/best_clf_valid_loss': float('inf'),
                'metric/best_clf_train': float('inf'), 'metric/best_clf_valid': float('inf'), 'metric/best_clf_test': float('inf'),
        ...
        # For classification, initialize with 0 since larger is better
        return {'metric/best_clf_epoch': 0, 'metric/best_clf_valid_loss': 0,
                'metric/best_clf_train': 0, 'metric/best_clf_valid': 0, 'metric/best_clf_test': 0,
                'metric/best_x_roc_train': 0, 'metric/best_x_roc_valid': 0, 'metric/best_x_roc_test': 0,
                'metric/best_x_precision_train': 0, 'metric/best_x_precision_valid': 0, 'metric/best_x_precision_test': 0}

init_metric_dict = get_init_metric_dict()
...
def get_local_config_name(model_name, dataset_name):
    if 'ogbg_mol' in dataset_name:
        ...
        local_config_name = f'{model_name}-default.yml'
    else:
